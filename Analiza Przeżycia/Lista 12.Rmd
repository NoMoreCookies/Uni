...
```
Age nie jest istotia z IW i Walda
...
Dla age p > 0.05 więc nieistotna zmienna
...
anova(update(m, . ~ . - as.factor(sex)),      m)  # IW dla sex (usuwa cały faktor naraz)
anova(update(m, . ~ . - as.factor(ph.ecog)),  m)  #
...
anova(update(m, . ~ . - age), m)  
anova(update(m, . ~ . - as.factor(sex)),      m)
anova(update(m, . ~ . - as.factor(ph.ecog)),  m)  
anova(update(m, . ~ . - ph.karno), m)  
