...
KM_z_ogonem <- function(times, status, name) {
  ...
  last_index <- c(last_index,length(last_index)+1) #muszę dodać jeszcze ten ostatni czas
  ...
  t_tail <- seq(t_plus, 2, length.out = 100)
...
w przypadku leku A jak i leku B jest taki sam i wynoi $e^{\frac{{ln\hat{S}(t^{+}})}{t^{+}}*t}$ gdzie w obu przypadkach $t^{+}=\frac{1}{2}$.
